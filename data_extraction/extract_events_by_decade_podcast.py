"""
Script to:
1) Fetch all episode descriptions for the Troy Story podcast from Apple Podcasts.
...
Podcast: https://podcasts.apple.com/us/podcast/troy-story-a-podcast-for-the-collar-city/id1725902540

Requirements:
    pip install google-generativeai requests feedparser
...
APPLE_PODCAST_ID = "1725902540"
ITUNES_LOOKUP_URL = f"https://itunes.apple.com/lookup?id={APPLE_PODCAST_ID}"

OUTPUT_CSV = "troy_story_podcast_decades.csv"  # Output CSV file
...
def fetch_podcast_feed_url() -> str:
    """
    Use the Apple iTunes Search API to look up the podcast and retrieve its RSS feed URL.
    """
    resp = requests.get(ITUNES_LOOKUP_URL, timeout=15)
    if resp.status_code != 200:
        raise RuntimeError(f"iTunes lookup failed with status {resp.status_code}")

    data = resp.json()
    results = data.get("results", [])
    if not results:
        raise RuntimeError("No podcast results returned from iTunes lookup.")

    feed_url = results[0].get("feedUrl")
    if not feed_url:
        raise RuntimeError("No feedUrl found in iTunes lookup result.")
    return feed_url


def fetch_podcast_descriptions(feed_url: str) -> str:
    """
    Fetch the podcast RSS feed and concatenate the descriptions from all episodes.
    """
    feed = feedparser.parse(feed_url)
    if getattr(feed, "bozo", 0):
        raise RuntimeError(f"Error parsing feed: {getattr(feed, 'bozo_exception', '')}")

    parts = []
    for entry in feed.entries:
        # Try several common fields in order
        text = (
            entry.get("summary")
    ...
    if not parts:
        raise RuntimeError("No episode descriptions found in RSS feed.")
...
def summarize_decade(model, decade: int, events):
    ...
    prompt = (
        ...
        f"Below are sentences from a podcast describing historical events "
        f"that occurred in the {decade}s.\n"
...
def main():
    print("Looking up Troy Story podcast feed URL from iTunes...")
    feed_url = fetch_podcast_feed_url()
    print(f"Feed URL: {feed_url}")

    print("Fetching episode descriptions from RSS feed...")
    descriptions_text = fetch_podcast_descriptions(feed_url)

    print("Extracting events by decade from episode descriptions...")
    decade_events = extract_decade_events(descriptions_text)
